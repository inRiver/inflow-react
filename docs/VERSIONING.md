...
| Compatibility tag | `react19-mui9.4` | A moving channel for the same React/MUI baseline. |
| Source tag | `theme/react19-mui9.4/v3.7.1` | Immutable Git tag for the source that produced a package version. |
...
```text
React 19 / MUI 9.4
dist-tag: react19-mui9.4
peer range: @mui/material >=9.0.0 <10.0.0
```

Previous checkpoints (still available for apps pinned to older MUI):
...
- `react19-mui9.3` (previous, MUI 9.3)
- `react19-mui9.4` (current, MUI 9.4)
- `react20-mui9.4` (future example)
...
| Follow a validated React/MUI checkpoint | `@inriver/inflow-react@react19-mui9.4` (current), `@inriver/inflow-react@react19-mui9.3` (MUI 9.3), or `@inriver/inflow-react@react19-mui7.3` (MUI 7) |
...
6. Push a `theme/<checkpoint>/vX.Y.Z` git tag (e.g. `theme/react19-mui9.4/v3.7.1`) only after the version is ready to release.
...
git tag -a "theme/react19-mui9.4/v$VERSION" -m "@inriver/inflow-react $VERSION - React 19 / MUI 9.4"
